...
| P4 | [pl-jobs-lora](https://github.com/P0w3r223/pl-jobs-lora) | QLoRA fine-tuning of a small Polish LLM on a self-built dataset from A2, compared honestly against zero-shot and few-shot API baselines | 🚧 Baselines measured, fine-tune pending | [🌐](https://p0w3r223.github.io/pl-jobs-lora/) |
| P5 | [doc-extract](https://github.com/P0w3r223/doc-extract) | structured extraction from Polish invoices where the point is not the extraction but the **error detection**: the Ministry's KSeF FA(3) schema is 183 798 bytes of XSD carrying 328 enumerations and **zero assertions**, so *net + VAT = gross* is unenforced by the national standard; 15 consistency rules fill that gap and are reported as data rather than raised, arithmetic identities counted apart from heuristics because a heuristic's false positives would otherwise be indistinguishable from a real miss; the corpus is generated so the gold needs no annotation step — KSeF-conformant XML *is* the ground truth, rendered to PDF across 9 difficulty tiers × 3 layouts and reproducible from one seed | 🚧 M1–6 of 7 done, most of M7 | [🌐](https://p0w3r223.github.io/doc-extract/) |
...
- **The published pages are held to a written specification, and an instrument here enforces part of it.** `python -m tools.pagespec` reads the **eleven committed surfaces**, prints a conformance table, and **refuses** when a gated clause fails; `--fetch` judges **all twelve** on the bytes they actually serve, and is the only way the twelfth is read at all — `wroclaw-air-insights` commits no HTML and exists only at its URL. The rules are [`docs/audit/0007`](docs/audit/0007_divergence-and-the-page-spec.md) § 5; `python -m tools.spec` prints every normative sentence and what carries it, including the ones nothing carries.
- **Every figure a page prints is a figure a committed artifact prints** — `0007` § 5.0. Applied to this file, honestly: the two *in progress* statuses above are levelled to each project's own README — `pl-jobs-lora`'s verbatim, `doc-extract`'s compressed to the cell's width from *"milestones 1–6 of 7, and most of the seventh"*. The `✅ Live` cells quote nothing and are this index's own shorthand: `mlops-car-price` says *"Complete."*, `apply-scout` *"complete — published"*, and `ab-lab` carries no status statement at all.
