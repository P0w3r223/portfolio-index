...
- **A2 · [it-job-radar](https://github.com/P0w3r223/it-job-radar)** — Polish IT job market: presence is observed for free and only attributes cost a fetch, demand is counted per *vacancy* rather than per advert (36 % of adverts repeat one job, which moved azure from third place to seventh), a figure is greyed when its bootstrap interval is too wide and not only when its sample is small, and a data contract refuses to publish data that fails it — with one SQL definition per metric, shown verbatim beside the figure it produced · [**live site**](https://p0w3r223.github.io/it-job-radar/)
...
| A2 | [it-job-radar](https://github.com/P0w3r223/it-job-radar) | data engineering — one sitemap request observes the whole population, so presence is never sampled and only attributes cost a fetch (bounded budget, at most once per offer); the unit of analysis is the *job*, not the advert, after measuring that 36 % of collected adverts are one role republished city by city — a bias that grows with the sample and had put azure third in the demand ranking instead of seventh; a published figure is withheld when we know it is false (an employer filed 14 500 PLN as an hourly rate; hourly rates filed as monthly were dragging five medians down) and greyed when its bootstrap interval is wider than a quarter of the median, not only when its `n` is small; versioned schema migrations; a normalization dictionary whose fixes reach data already stored and which refuses to load if one technology would land in two buckets; a data contract enforced before anything is published; one SQL definition per published metric, run in DuckDB over a redacted Parquet artifact and shown to the reader verbatim; every figure carries its `n`, and metrics are recorded dated — renamed rather than continued when their definition changed | ✅ Live | [🌐](https://p0w3r223.github.io/it-job-radar/) |
