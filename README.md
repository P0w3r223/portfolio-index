...
- **P1 · [mlops-car-price](https://github.com/P0w3r223/mlops-car-price)** — the A3 model kept alive: versioned data, MLflow registry, an *evaluated* drift detector, and promotion decided by a paired bootstrap

🚧 In progress: the flagship **[apply-scout](https://github.com/P0w3r223/apply-scout)** (P3) — an LLM agent with a trajectory-evaluation harness (milestone 1/7: data contracts + a from-scratch tool loop, tested under a scripted model).
...
| P3 | [apply-scout](https://github.com/P0w3r223/apply-scout) ⭐ | flagship — an LLM agent written from scratch (tool loop, budgets, guardrails) with a trajectory-evaluation harness: success rate, citation fidelity, cost per task | 🚧 In progress (1/7) |
