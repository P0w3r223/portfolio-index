...
settled. It confirmed every structural finding above, and found five things about the
...
**382 of 533** commits across the twelve public repositories carry `Co-Authored-By: Claude` or
"Generated with Claude Code". It is visible on every commit and every pull-request page. The
distribution is uneven — `token-budget` 9/9, `doc-extract` 37/40, `pl-jobs-lora` 38/44, against
`auth-log-scan` 4/13 and `mini-traceroute` 4/12.

Four repositories have run pull requests explicitly titled *"drop the Claude co-author line"*, so
this was already a live concern — but only the knowledge documents were cleaned. Whether to
disclose is the author's decision and there is a defensible case either way. The **finding** is the
inconsistency: the documents say one thing about how this work was made and the commit history says
another.

### 6.3 The new flagship's commits are not credited to the account

**37 of `doc-extract`'s 40 commits** are authored as `biap.claude@gmail.com`, an address not
