...
reachable from their `origin/main`. Index `main` at `3bf7b5a`. The only working-tree noise is untracked local
`.claude/` directories.
...
|---|---|---|---|---|
| **S0** | Split `0007`; accept §5 / §5.1 / clause 9 as normative; freeze §3; move §9 here; record §2.1's refuted figures; take the carrier decision (`ADR-0004`) | index only, no code | 0.5 d | **closed** — `current_projects` `5ebee62` on `main` |
...
different numbers, which is the argument for not typing a fourth. `8f270cb` already moved one table in that
...
equal *and* on `main`, working tree clean apart from untracked `.claude/`. Index `main` at `c198d54`.
...
`current_projects` `67fc10e`, on `main`. **No page changed**, which was the argument for taking it first: the ratchet
...
branch SHA survives. Checked — `5ebee62` (S0), `8f270cb` (S2), `a56e48e` (S3) and `5c74669` (S4) are all
reachable from `main`; `043bf72`, `38f26a9` and `3163908` are not and never will be. The stage *was* merged,
as `#77` → **`67fc10e`**, with CI green on `main` — so this is not the S4 defect the row above it records, a
...
**Measured before and after at `62499da`:** 153 → **88**; `auth-log-scan` 101 → 36 and the other
...
**Measured before and after at `0c33a71`.** 88 → **26**. `it-job-radar` goes clean — its three
...
| **The quotation rule has no carrier on `README.md`** | §4.10 applied `0007` §5.0 to `docs/index.html` and found ten of twelve READMEs carrying a figure no artifact prints. The figures were repaired; **nothing detected the next one until 2026-09-11.** `ab-lab` generates and byte-guards README regions (`<!-- generated: -->` at `README.md:14, 45, 49, 92, 96`); `auth-log-scan` now runs the command its README names and compares the block byte for byte, and **since `bfe4bf2` also re-derives the two figures its README states about the repository** — the demo log's length, and the number of tests that need no log (`0010` §4 row A-1 and its errata). *This cited `1d02f6f` alone until 2026-09-11; the second guard landed in the pull request that bumped this pointer, which is the shortest life a citation in this row has had.* **Two carriers of different shapes, and the rest are still hand-typed prose with none** — the same structural shape as §4.11's two clauses, and it wants the same answer, but a README figure-provenance reader is a larger build than a separator and should follow S9 rather than ride inside it. *This row read* **only `ab-lab`** *and* **twelve of thirteen** *until 2026-09-11, and the correction had to be made in three files — here, `0009` §14.3 and `tools/spec.py` `q.s1` — which is ST-3 arriving on schedule. No count replaces it: the repair that falsified this one was itself found falsifying a hand-typed test count in the README it was repairing* |
