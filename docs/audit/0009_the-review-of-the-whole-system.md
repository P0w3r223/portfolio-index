...
| 12 | **Contrast at the usage site** — a clause using the existing `colour.resolve()`/`composite()`, `UNDECIDED` for `color-mix`, `opacity < 1` and SVG paint order, threshold per usage site | N4, `0007` §7's largest gap | M | open, and **next after row 13b** — it is the only open row where a published page can harm a reader, which is §3.1's promotion rule. ~~Ships report-only with a `NOT_A_CLAUSE` entry stating that as a decision~~ — **that route was refused by the guard beside it**, found 2026-09-08 by the architecture pass on this row: the floor's exemption check admits no key the corpus reports `PASS` or `FAIL`, and a contrast clause passes on a conforming page. It ships as `Ratchet(prefix, REPORT_ONLY_STATE, reason)` in `__main__.GATE`, which is what row 13b built and why that row was due first. **Row 13b is now closed.** The row was then *opened by measurement* rather than by building: `0008` S12 took the finding and the instrument's own blind spot, and left the clause itself as **S13**. Three claims did not survive the measurement — `--accent-soft` on `--surface` is not live anywhere (35 `<svg>`, none inside a `.card`), a rule-keyed clause is 59 findings of which ~0 are real, and the one live SC 1.4.3 failure was in a **presentation attribute** neither carrier read. `0008` §4.19 |
