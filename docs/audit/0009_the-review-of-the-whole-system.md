...
workflow that runs all three — taken 2026-09-07 against `main` at `95cebec`. Three passes ran in
...
`main` at `95cebec`; twelve submodule pointers equal to their own `origin/main` after a `git fetch`;
no open pull request in any of the thirteen repositories; CI green on `main` at `2656825`; the
...
4. **`autoMode.environment` in the user's global settings carries another project's facts**,
   ...
   operating here reads two incompatible descriptions of the world it acts in. **Closed
   2026-09-11**: `autoMode.environment` went 41 entries → 24, and the seventeen removed were one
   block of thirteen Org-wide rows and four User-specific ones, all of them the other project's.
   *The command is written beside the figure on `0010` R-1's precedent, because there is no
   commit to cite and nothing else can re-derive it:*
   `python -c "import json,os;print(len(json.load(open(os.path.expanduser('~/.claude/settings.json'),
   encoding='utf-8'))['autoMode']['environment']))"`
   `autoMode.soft_deny` still names that project and was **deliberately left**: it is a consent
   requirement before a production run against real personal data, so removing it would be
   weakening a guard to tidy a file. A factual correction rather than a new rule, as the row said.
...
|---|---|---|---|---|
| 1 | Session-start hook + a root `CLAUDE.md` | §6.2 rows 1–3 | S | **done — `#81`, `7c13995`** |
...
| 9 | Split `0008`: ledger, **a failure-taxonomy document**, measurements to the report | §6.2 row 5 | ~~S~~ **M**, and it needs a design decision first | open — **scope re-derived 2026-09-10 at `bcd53db`, before the stage rather than during it, which is §4.7's precedent for the sixth time. The row is neither the size nor the urgency it carries.** *Not `S`.* §4 is **2 597 of 3 207 lines**, and **71 citations of `0008` §4.x live in 18 tracked files outside `0008`**, measured at `bcd53db`: `tools/pagespec/__main__.py` 9, `tests/test_report.py` 9, `tests/test_clauses.py` 8, `tools/spec.py` 7, this file 7, `ADR-0008` 7, `tools/pagespec/clauses.py` 6, `ADR-0007` 5, `0007` 2, `CLAUDE.md` 2, `.github/workflows/pagespec.yml` 2, and seven files with one apiece — production code, the CI workflow and the contributor map among them. **The submodules are not reached**, which is the one fact keeping this an index-only stage: they cite `0008` §3.6 and nothing else (`apply-scout` 3, `auth-log-scan` 2), and §3 stays in the ledger. *The first version of this figure read 81 across 22 files, ten of them in four session briefs, and argued that those ten must not move. They are not in the repository at all: `.claude/sessions/` is gitignored at `.gitignore:25` and `git ls-files` returns none of them, so they are one machine's local state — nothing to re-point and nothing a clone has. The figure was taken from the filesystem where every other figure in this row comes from git.* **And 71 is a floor rather than the scope.** It matches on the literal string `0008`, and this document is also cited **bare**: `render.py:98`, `CLAUDE.md:158`, `ADR-0005:26`, and `ADR-0006:9` twice — five sites carrying no document number, which a mechanical re-point cannot find by grepping `0008` at all. **69 bare `§4.x` sit in 19 tracked files** whose line names no document; most are a document citing itself, and telling those apart from `0008`'s takes a read per site. That is the strongest argument in this row for `@architect`, and it is that the mechanical half is not mechanical. Same shape as row 13b, which this table also sized `S` and which §7's own note admits was *"reasoned rather than run"* — the second time this column has been wrong the same way, which is what makes it a pattern rather than an estimate. *And the reason to take it next has expired.* The row argues from **2 197 → 2 470**, today **3 207**, and the file grew because **each stage added a §4.x section**. `0008` now has no open stage: S8c is the only row not `closed` and it is a refusal on measurement, not scheduled work. The growth stopped when S14b closed, so the waiting stopped costing at the same moment — the argument for urgency expired on the day it was loudest. *What is left is a design decision and not a move.* The row names three destinations, and a §4.x section mixes all three inside one paragraph: *"What closing SX measured"* carries status, a recurring failure class and figures at once. Re-pointing 81 citations is mechanical only if the section numbers survive, and they cannot survive a partition that reads for meaning. `@architect` before `@Plan`. **Answered 2026-09-11 by [`ADR-0009`](../adr/0009_the-ledger-is-addressed-not-partitioned.md), now `accepted`. Steps 1 and 2 have landed — `#126` the resolver, `#127` the CI half that makes it run on the class of change it watches, and `0008` §4.11's three gate policies moved to `ADR-0006` §7. **Step 3 landed the same day**: `docs/reference/failure-classes.md` names ten classes in four families, each with where it was defined, where it recurred and the test that catches the next one — and the occurrence lists are computed from the record's own recurrence markers rather than read for. The row stays `open` for **step 4 alone**, which is opportunistic by construction: the code-side citations move onto class ids when their file is next touched for another reason, never in a sweep. **That population is printed on every run** — `python -m tools.citations`'s `code-side` row, 104 citations in 25 files at `b4177d4` — and this row closes when a reader judges it small enough, not on a threshold, because not every citation of the ledger from code is wrong and no instrument can tell the two apart. *An earlier version of this cell said the row closes when the resolver reports the migration done; the resolver emitted no such signal, which is `FG-1` committed in a sentence about `FG-1`.* That decision refuses the three-way split and takes a fourth option: build the citation resolver first, promote §4.11's gate policies to `ADR-0006`, name the failure classes in `docs/reference/`, renumber nothing. **Two of this row's three destinations do not exist**, which the answer measures rather than argues — the taxonomy is defined in §3.7, §3.9 and §3.10, which this same row keeps in the ledger, and the report prints today's census while §4 records a history of them. **And this row's submodule evidence is refuted**: `git grep` in all twelve trees finds eleven sites in four repositories, not five in two, with `auth-log-scan` citing §3.9 and §3.10 rather than §3.6 — though the conclusion it supports survives, because no submodule reaches §4 at all. The 71-in-18-files figure this row calls a floor is one: 162 lines carry a `§4.x` outside the ledger, 89 of them naming `0008` in their content |
| 10 | **Sx** — 11 `pyproject.toml`, 70 `Author:` fields | dead weight | 0.5 d | **done** — eleven sibling pull requests plus `9ed402a` and `509a469`. Both figures now read **0**. `509a469` is green on `main`; `9ed402a` is `docs/**` only, which the paths filter excludes, so it has no run. `0008` §4.14 records what the stage measured, including a deprecation date the record did not have |
| 11 | Bring the **profile README** into the system — ~~at minimum a `Surface` read by `--fetch` in `live`~~, plus the quotation rule | the asymmetry in §9 | **M, not S** | open, and **re-scoped by measurement 2026-09-08** — §14. **Still open on 2026-09-17, and the stake rose:** the profile was edited by hand that day to link this repository (`P0w3r223/P0w3r223#4`), so it now carries traffic *into* the index while remaining checked by nothing. The edit was verified by hand — target 200 anonymously, the private origin 404 — which is exactly the manual step this row exists to replace |
...
| 13 | Registry-drift test (N1) · resolve N3 (implement the geometry half or amend `ADR-0004` §4 to say it is deferred and unowned) · clean `autoMode.environment` | N1, N3, §6.2 row 4 | S | **N1 and N3 done — 2026-09-08.** N1: the workflow's two `paths:` filters are tied to `sources.SURFACES` by a `core` test, which is the half §12.4 leaves and the one this row correctly calls the silent failure. N3: deferred and unowned, in `ADR-0004` §4 and `tools/spec.py` `c3.s2` — **its evidence did not reproduce**, §8 row 7. **`autoMode.environment` done 2026-09-11** — 41 entries → 24, seventeen of the other project's removed, `soft_deny` left alone because it is a consent requirement and not a description. It is the user's global settings rather than this repository, so the record is §6.2 row 4, there is no commit to cite, and the count is a figure no instrument here can re-derive — §6.2 row 4 carries the command that produced it. **The row is closed** |
...
***Settled and implemented 2026-09-07 — `#86`, `859190c`. The paragraph below is the question as it stood; `named_after_the_directory` no longer exists and the answer is four surfaces, computed rather than asserted.***
...
`76e0396`, before any of the day's work. `clauses.py`'s own comments on `_LENGTH` and
...
*This is `0008` §5's carried README row — most READMEs hand-typed with no carrier — arriving
at its worst instance. The row says a provenance reader "should follow S9 rather than ride
inside it." S9 has now landed, and this is the surface that should be its first target rather
than its last. **Two repositories carry a README guard as of 2026-09-11** — `ab-lab`'s
generated regions and `auth-log-scan`'s quoted command — and the profile is reached by
neither, because nothing in this repository reaches it at all, which is §7 row 11.*
...
| point | collected |
|---|---|
| `207b1db`, S13's entry | 519 |
| `bcb479d`, S13 merged and the pointers bumped | 576 |
| `de6e4f3`, the audit's first four commits (`#109`) | 600 |
...
documentation-only pull request receives no run — `0008`'s Sx row recorded that for `9ed402a`
