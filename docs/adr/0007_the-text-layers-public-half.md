...
| **W3** | One index pull request: twelve pointer bumps + L3 (`README.md:29-37`) + H3 | after W1 | `pagespec` runs, and the conformance table must be **byte-identical** against the W0 baseline — no page's verdict may move. **Done 2026-09-09**, `#104`. Both gating CI jobs green, `the live surface` correctly skipped |
| **W4** | The profile README pull request — H3's public half | with W3 | review only; nothing carries this surface. **Done 2026-09-09**, `P0w3r223/P0w3r223` `#3`. The wording was copied from the index and the About rather than composed again, which is the finding H3 reports |
